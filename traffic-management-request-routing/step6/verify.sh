...
check_output_contains "kubectl exec -it tester -- bash -c 'for i in {1..20}; do curl -s -X POST http://notification-service/v2/notify; echo; done;'" SMS
check_output_contains "kubectl exec -it tester -- bash -c 'for i in {1..20}; do curl -s -X POST http://notification-service/v2/notify; echo; done;'" EMAIL
